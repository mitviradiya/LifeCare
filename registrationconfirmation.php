...
<body>
  <?php 
	         $query = "select username from tbllogin";
	   $res = mysql_query($query);
	   $row = mysql_fetch_array($res);
		if($_REQUEST["txtusername"]==$row[0])
	   {
		   $i= "User Allready Registered";
	   }	 
	else	
	{
	   $query="insert into tbluser(firstname,lastname,address,city,pincode,mobile,email,gender,birthdate) values('".$_REQUEST["txtfirstname"]."','".$_REQUEST["txtlastname"]."','".$_REQUEST["txtaddress"]."','".$_REQUEST["txtcity"]."','".$_REQUEST["txtpincode"]."','".$_REQUEST["txtmobile"]."','".$_REQUEST["txtemail"]."','".$_REQUEST["gender"]."','".$_REQUEST["txtbirthdate"]."')";
	   mysql_query($query);
	   
	   $query = "select max(userid) from tbluser";
	   $res = mysql_query($query);
	   $row = mysql_fetch_array($res);
	   
	   
	   $query = "insert into tbllogin(username,password,securityquestion,securityanswer,authentication,usertype,userid) values ('".$_REQUEST["txtusername"]."','".$_REQUEST["txtpassword"]."','".$_REQUEST["txtquestion"]."','".$_REQUEST["txtanswer"]."','True','customer',".$row[0].")";
	   
	   mysql_query($query);
	   $i="User Succesfully Registered";
	}

	  // $photo= "ProfileImage/".$_FILES["filephoto"]["name"];

//move_uploaded_file($_FILES["filephoto"]["tmp_name"],$photo);
	   
	  //echo "<script> alert('User Inserted Successfully'); document.location='login.php' </script>";
		?>
  ...
  <div class="container">
    <div class="row">
      <div class="col-md-12">
         ...
         <div class="page_link"><a href="index.html">Home</a><span><i class="fa fa-long-arrow-right"></i>Register</span></div>
      </div>
    </div>
  </div>
</div>  
  ...
  <div class="container">
    <div class="row">
      <div class="col-md-8 col-sm-8">
		...
		<form class="callus" method="post" action="login.php"  id="contact_form">
        <h2> <?php echo $i; ?></h2><br><br>
		<div class="form-group">
                 <div class="btn-submit button3">
              
		<input type="Submit" name="back" value="Back to Login" id="btn_contact_submit"/>
		</div>
		</div>
		</form>
    ...
    <div class="row">
     <div class="col-md-12">
        <div class="copyright clearfix">
          <p>Copyright &copy; 2016 Mit Viradia & Team. All Right Reserved</p>
           <ul class="social_icon">
